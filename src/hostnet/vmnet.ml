...
let failf fmt = Fmt.kstr (fun e -> Error (`Msg e)) fmt

type t = {
  mutable fd: Flow.two_way option;
  ...
  pcap_m: Eio.Mutex.t;
  mutable listeners: (Cstruct.t -> unit) list;
  mutable listening: bool;
  after_disconnect: unit Eio.Promise.t;
  after_disconnect_u: unit Eio.Promise.u;
  ...
  mutable callback: (Cstruct.t -> unit);
  log_prefix: string;
}
...
let server_negotiate ~fd ~connect_client_fn ~mtu =
  let assign_uuid_ip uuid ip =
    let mac = connect_client_fn uuid ip in
    match mac with
    | Error (`Msg msg) ->
    ...
    | Ok client_macaddr -> 
        ...
        Flow.write fd [ buf ];
        Ok (uuid, client_macaddr)
  in
  let buf = Cstruct.create Init.sizeof in 
  Flow.read_exact fd buf;
  ...
  | x -> 
    ...
    Error (`Msg "Client requested unsupported protocol version")
...
let client_negotiate ~uuid ?preferred_ip ~fd () =
  ...
  | 22l -> 
      ...
      let buf = Cstruct.create Init.sizeof in
      Flow.read_exact fd buf;
      Response.unmarshal buf >>= fun (response, _) ->
      (match response with
      | Vif vif -> 
  ...
  | x -> 
      ...
      Error (`Msg "Server does not support our version of the protocol")
...
let make ~client_macaddr ~server_macaddr ~mtu ~client_uuid ~log_prefix fd =
  ...
  let pcap_m = Mutex.create () in
  let listeners = [] in
  let listening = false in
  let after_disconnect, after_disconnect_u = Promise.create () in
  let callback _ = () in
...
| Some fd ->
  ...
  Promise.resolve t.after_disconnect_u ()
...
let with_read t x f =
  match x with
  | Error e      -> err_unexpected t Format.pp_print_string e
  | Ok `Eof      -> err_eof t
  | Ok (`Data x) -> f x
...
let listen_nocancel ~sw t new_callback =
  ...
  let rec loop () =
    (with_fd t @@ fun fd ->
      ...
      Flow.read_exact fd read_header;
      ...
      Log.debug (fun f ->
          ...
          List.iter (Cstruct.hexdump_to_buffer b) [ buf ];
          f "received%s" (Buffer.contents b)
        );
      let callback buf =
        try t.callback buf with
          | e ->
      ...
      in
      Fiber.fork ~sw (fun () -> callback buf);
  ...
  begin
    if not t.listening then begin
      ...
      Promise.await t.after_disconnect;
...
let listen ~sw t ~header_size:_ new_callback =
  (* let task, u = Promise.create () in *)
  ...
  let _ =
    (* TODO: Cancel.protect ? *)
    listen_nocancel ~sw t new_callback
    (* Promise.resolve u x *)
  in
    Ok ()

let write t ~size fill =
  Mutex.use_ro t.write_m (fun () ->
