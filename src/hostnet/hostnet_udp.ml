...
struct
  ...
  let start_background_gc ~mono ~clock table by_last_use max_idle_time new_flow_lock =
    let rec loop () =
      Eio.Time.sleep clock (Int64.to_float max_idle_time /. 1_000_000_000.);
      Mutex.use_ro new_flow_lock
        (fun () ->
          ...
          in
          List.iter (expire table by_last_use) to_shutdown
        );
      loop ()
    in
    loop ()

  let create ?(max_idle_time = Duration.(of_sec 60)) ?(preserve_remote_port=true) ?(max_active_flows=1024) ~sw net mono clock () =
    let table = Hashtbl.create 7 in
    let by_last_use = ref By_last_use.empty in
    let new_flow_lock = Mutex.create () in
    let background_gc_t = start_background_gc ~mono ~clock table by_last_use max_idle_time new_flow_lock in
    let send_reply = None in
    { sw; net; mono; clock; max_idle_time; max_active_flows; new_flow_lock; background_gc_t; table; by_last_use; send_reply; preserve_remote_port }
  ...
  let outside_to_inside t flow server d =
    ...
    let rec loop () =
      let v = 
       try
        let (n, from) = Udp.recvfrom server buf in
        touch t flow;
  ...
  let expire_old_flows_locked t =
    ...
    then ()
      ...
      let count = loop to_delete seq 0 in
      Log.info (fun f -> f "Expired %d UDP NAT rules" count)
    end

  let input ~t ~datagram ~ttl () =
    let d = description datagram in
    let v = if Hashtbl.mem t.table datagram.src then begin
        Some (Hashtbl.find t.table datagram.src)
      end else begin
          ...
          try
            Mutex.use_ro t.new_flow_lock
              (fun () ->
                ...
                then Some (Hashtbl.find t.table datagram.src)
                else begin
                  expire_old_flows_locked t;
                  let server = Udp.bind ~sw:t.sw t.net ~description:(description datagram) (Ipaddr.(V4 V4.any), 0) in
                  ...
                  (* Start a listener *)
                  Fiber.fork ~sw:t.sw (fun () -> outside_to_inside t flow server datagram);
                  (Some flow)
                end
              )
          with e ->
